...
## 10) How to read any MMA variable from MEGA (exact columns + queries)

### 10.1 Where MMA data lives (rule)
For a given `generation_id`:

1) **Final “canonical variable map”** is stored on the **generation row**:
- Table: `MEGA_GENERATIONS`
- Row: `mg_id = 'generation:<generation_id>'` (or `mg_record_type='generation' AND mg_generation_id='<generation_id>'` if you use that)
- Column: `mg_mma_vars` (JSONB)

2) **Step-by-step pipeline inputs/outputs** are stored as **`mma_step` rows**:
- Table: `MEGA_GENERATIONS`
- Rows: `mg_record_type='mma_step' AND mg_generation_id='<generation_id>'`
- Columns:
  - `mg_step_type` (which stage)
  - `mg_payload` (JSONB containing `input` and `output` for that step)

✅ If you want to answer questions like **“what was Input_gpt_reader?”**, you read the **`mma_step` row for `mg_step_type='gpt_reader'`** and extract it from `mg_payload.input`.

---

### 10.2 Step types you MUST write (so reads are deterministic)
These `mg_step_type` values should be used exactly (string stable contract):
...
**Video animate**
- `scan_input_still`
- `motion_suggestion` (only if “Type for me”)
- `gpt_reader_motion`
- `kling_generate`
- `postscan_output_video` (optional if you later scan video)
...
### 10.3 Required mg_payload shape per `mma_step` (so you can always read it)
For **every** `mma_step`, store:
...
And for the steps below, you MUST include these keys:

#### A) `gpt_reader` (this is where `Input_gpt_reader` lives)
**Write:**
- `mg_payload.input.input_gpt_reader` (STRING): concatenation you actually fed GPT reader  
- `mg_payload.input.parts` (OBJECT): the structured parts (recommended)
- `mg_payload.output.clean_prompt` (STRING): output prompt
- `mg_payload.output.userMessage_final` (STRING): final user message line (optional)
...
```json
{
  "input": {
    "parts": {
      ...
      "userBrief": "I want ...",
      "style_history_csv": "editorial still life, minimal, no light flare"
    },
    "input_gpt_reader": "<product_crt> <logo_crt> <inspiration_crt> <style> <userBrief> <style_history_csv>"
  },
  "output": {
    "clean_prompt": "CLEAN PROMPT ...",
    "userMessage_final": "Done — I built a clean prompt for you."
  }
}
```

#### B) `like_history` (this is where `Gpt_like_history` lives)
**Write:**
- `mg_payload.input.window` (INT): 5 or 20
- `mg_payload.output.style_history_csv` (STRING): the comma-separated preferences
- `mg_payload.output.style_history_tags` (ARRAY): parsed tags (recommended)

#### C) Scan steps (this is where `*_crt` lives)
For `scan_product`/`scan_logo`/`scan_inspiration`/`scan_input_still`:
**Write:**
- `mg_payload.output.crt` (STRING)
- `mg_payload.output.userMessage_line` (STRING) if you want one streaming line per scan

#### D) Generation steps (Seedream/Kling)
For `seedream_generate` / `kling_generate`:
**Write:**
- `mg_payload.input.prompt` (STRING)
- `mg_payload.output.output_url` (STRING) and/or `output_key`

---

### 10.4 SQL recipes (copy-paste)

#### 10.4.1 Get the canonical MMA map (fast path)
```sql
select mg_mma_vars
from public.mega_generations
...
#### 10.4.2 Get **Input_gpt_reader** (exact answer)
```sql
select
  mg_payload #>> '{input,input_gpt_reader}' as input_gpt_reader,
  mg_payload #>  '{input,parts}'          as parts_json
from public.mega_generations
where mg_record_type = 'mma_step'
  and mg_generation_id = $1
...
#### 10.4.3 Get **Output_gpt_reader** (clean prompt)
```sql
select
  mg_payload #>> '{output,clean_prompt}' as clean_prompt
from public.mega_generations
where mg_record_type = 'mma_step'
  and mg_generation_id = $1
  and mg_step_type = 'gpt_reader'
...
#### 10.4.4 Get scan captions (product/logo/inspirations)
```sql
select mg_step_type, mg_payload #>> '{output,crt}' as crt
from public.mega_generations
where mg_record_type='mma_step'
  and mg_generation_id = $1
  and mg_step_type in ('scan_product','scan_logo','scan_inspiration','scan_input_still')
order by mg_step_no asc;
```

#### 10.4.5 Get the streamed loading lines (userMessages)
If you store them in canonical map:
```sql
select mg_mma_vars #> '{userMessages,scan_lines}' as scan_lines
from public.mega_generations
where mg_id = 'generation:' || $1;
```

If you store one line per scan step:
```sql
select mg_step_no, mg_payload #>> '{output,userMessage_line}' as line
from public.mega_generations
where mg_record_type='mma_step'
  and mg_generation_id = $1
  and mg_payload ? 'output'
order by mg_step_no asc;
```

---

### 10.5 Frontend “where do I read X?” (quick mapping)
| MMA variable you ask about | Read from | Exact location |
|---|---|---|
| `Input_gpt_reader` | `MEGA_GENERATIONS` | `mma_step` row where `mg_step_type='gpt_reader'` → `mg_payload.input.input_gpt_reader` |
| `Output_gpt_reader.prompt` | `MEGA_GENERATIONS` | same step → `mg_payload.output.clean_prompt` (and also `generation` row `mg_prompt`) |
| `product_crt` | `MEGA_GENERATIONS` | `mma_step` where `mg_step_type='scan_product'` → `mg_payload.output.crt` |
| `logo_crt` | `MEGA_GENERATIONS` | `mma_step` where `mg_step_type='scan_logo'` → `mg_payload.output.crt` |
| `inspiration_crt[]` | `MEGA_GENERATIONS` | `mma_step` where `mg_step_type='scan_inspiration'` → `mg_payload.output.crt` (multiple rows or array) |
| `Gpt_like_history` / `style_history_csv` | `MEGA_GENERATIONS` | `mma_step` where `mg_step_type='like_history'` → `mg_payload.output.style_history_csv` |
| `userMessage_i` lines | `MEGA_GENERATIONS` | either `generation.mg_mma_vars.userMessages.scan_lines[]` OR step-wise `mg_payload.output.userMessage_line` |
| `still_crt` (input still caption) | `MEGA_GENERATIONS` | `mma_step` where `mg_step_type='scan_input_still'` → `mg_payload.output.crt` |
| `output_still_crt` (output image caption) | `MEGA_GENERATIONS` | `mma_step` where `mg_step_type='postscan_output_still'` → `mg_payload.output.crt` |
| `input_gpt_feedback` | `MEGA_GENERATIONS` | `mma_step` where `mg_step_type='gpt_feedback_still'` → `mg_payload.input...` |
| `output_gpt_feedback.prompt` | `MEGA_GENERATIONS` | same → `mg_payload.output.prompt` |
| `prompt_motion` | `MEGA_GENERATIONS` | `mma_step` where `mg_step_type='gpt_reader_motion'` → `mg_payload.output.motion_prompt` |

## 11) Config versioning (ctx_* + provider presets + adding providers)

This section answers exactly:
- **“If I want to tweak context ctx_... of any GPT what to do?”**
- **“If I want to tweak predefined settings of AI what to do?”**
- **“If I want to add other AIs what to do?”**

### 11.1 Rule: configs live in MEGA_ADMIN, runs store the exact version used
You must **version** configs (never edit in-place), and every MMA run must persist:
- the **config key** (ctx/preset/registry)
- the **config version**
- optionally a **hash** of the rendered template/settings

**Where:**
- Store configs: `MEGA_ADMIN` (`mg_record_type='app_config'`)
- Store “what was used”: `MEGA_GENERATIONS` (`mg_record_type='mma_step'`) inside `mg_payload.input`

### 11.2 Standard config keys (stable naming contract)
Use these keys exactly to avoid chaos:
...
**Provider registry**
- `mma.provider.registry` (which providers are enabled + capabilities)

### 11.3 How to store configs in MEGA_ADMIN (rows)
Each version is its own row (simple, immutable):

- `mg_id`: `app_config:<key>.v<version>`
  - Example: `app_config:mma.ctx.gpt_reader.v4`
- `mg_record_type`: `app_config`
- `mg_key`: the stable key (e.g., `mma.ctx.gpt_reader`)
- `mg_value`: JSON with at least `{ "version": <int>, "enabled": true, ... }`

**Example: GPT reader context v4**
```sql
insert into public.mega_admin (mg_id, mg_record_type, mg_key, mg_value, mg_created_at, mg_updated_at)
values (
  'app_config:mma.ctx.gpt_reader.v4',
  'app_config',
  'mma.ctx.gpt_reader',
  '{
    "version": 4,
    "enabled": true,
    "system": "You are MMA GPT reader...",
    "user_template": "Input: {{input_gpt_reader}}",
    "output_schema": { "type":"object", "properties": { "clean_prompt": { "type":"string" } } }
  }'::jsonb,
  now(),
  now()
)
on conflict (mg_id) do update
set mg_value = excluded.mg_value,
    mg_updated_at = now();
```

**Example: Seedream defaults v3**
```sql
insert into public.mega_admin (mg_id, mg_record_type, mg_key, mg_value, mg_created_at, mg_updated_at)
values (
  'app_config:mma.provider.seedream.defaults.v3',
  'app_config',
  'mma.provider.seedream.defaults',
  '{
    "version": 3,
    "enabled": true,
    "defaults": { "quality": "high", "aspect_ratio": "1:1" }
  }'::jsonb,
  now(),
  now()
)
...
### 11.4 How backend fetches “active config” (latest enabled version)
Backend selects the highest enabled version per `mg_key`:

```sql
select mg_id, mg_value
from public.mega_admin
where mg_record_type = 'app_config'
  and mg_key = $1
  and coalesce((mg_value->>'enabled')::boolean, false) = true
order by (mg_value->>'version')::int desc
limit 1;
-- $1 = 'mma.ctx.gpt_reader' (or any key)
```

### 11.5 How to persist “config used” per GPT call (so you can debug later)
For any GPT step (example `gpt_reader`), write into the `mma_step` row:

- Table: `MEGA_GENERATIONS`
- Row: `mg_record_type='mma_step' AND mg_step_type='gpt_reader'`
- Column: `mg_payload`
...
```json
{
  "input": {
    ...
    "ctx_hash": "sha256:...optional...",
    "input_gpt_reader": "...",
    "parts": { "...": "..." }
  },
  "output": {
    "clean_prompt": "...",
    "userMessage_final": "...optional..."
  }
}
```

✅ Then: “What ctx did GPT reader use?” is answered by reading:
- `mg_payload.input.ctx_key`
- `mg_payload.input.ctx_version`
- `mg_payload.input.ctx_id`

### 11.6 How to persist “resolved provider settings” per generation
Backend resolves settings by deep-merging:
1) provider defaults (from `MEGA_ADMIN`)
2) customer prefs (optional: `MEGA_CUSTOMERS.mg_mma_preferences`)
3) frontend overrides (request payload)

Then persist:
- on the **generation row**: `mg_mma_vars.settings.<provider>`
- on the **generate step row**: `mg_payload.input.settings_resolved`

Example JSON path:
- `generation.mg_mma_vars -> settings -> seedream`
- `mma_step(seedream_generate).mg_payload -> input -> settings_resolved`

### 11.7 Adding a new AI provider (no schema changes)
To add provider `<provider>`:

1) Add defaults config row:
- `mg_id='app_config:mma.provider.<provider>.defaults.v1'`
- `mg_key='mma.provider.<provider>.defaults'`
- `mg_value.defaults={...}`

2) Add/adjust GPT translator context if provider needs special formatting:
- either reuse `mma.ctx.gpt_reader` with `target_provider=<provider>`
- or add `mma.ctx.gpt_reader.<provider>` as a separate key

3) Add a generator step type (string contract):
- `<provider>_generate` (e.g., `runway_generate`)
- Persist provider request/response in `mg_payload`

4) Update provider registry:
- key: `mma.provider.registry`
...
```json
{
  ...
  "enabled": true,
  "providers": [
    { "id": "seedream", "modes": ["still"], "enabled": true },
    { "id": "kling", "modes": ["video"], "enabled": true },
    { "id": "runway", "modes": ["video"], "enabled": false }
  ]
}
```

### 11.8 “How do I tweak ctx_* or settings?” (one-liner)
- **Create a new version row** in `MEGA_ADMIN` (`...vN+1`), set `enabled=true`, optionally disable older versions.
- New runs automatically pick the latest enabled version.
- Old generations remain reproducible because they stored `ctx_version` and settings used in their `mma_step` rows.
