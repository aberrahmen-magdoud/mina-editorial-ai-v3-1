...
function createMmaRouter() {
  ...
  async function withPassId(req, rawBody) {
    const body = rawBody && typeof rawBody === "object" ? rawBody : {};
    const authUser = await getAuthUser(req);
    const authPassId = authUser?.userId
      ? normalizeIncomingPassId(`pass:user:${authUser.userId}`)
      : "";
    const passId = authPassId || resolvePassId(req, body);
    return { passId, body: { ...body, passId } };
  }

  router.post("/still/create", async (req, res) => {
    const { passId, body } = await withPassId(req, req.body);
  ...
  app.use("/mma", async (req, res, next) => {
    try {
      ...
      const authPassId = authUser?.userId
        ? normalizeIncomingPassId(`pass:user:${authUser.userId}`)
        : "";
      const resolved = normalizeIncomingPassId(resolvePassId(req, req.body || {}));
      const passId = authPassId || resolved;
      setPassIdHeader(res, passId);
