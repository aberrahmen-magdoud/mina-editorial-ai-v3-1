...
function appendScanLine(vars, text) {
  ...
  const t = typeof text === "string" ? text : safeStr(text?.text, "");
  if (!t) return next;
...
function makePlaceholderUrl(kind, id) {
  const base = safeStr(process.env.R2_PUBLIC_BASE_URL, "https://example.r2.dev");
